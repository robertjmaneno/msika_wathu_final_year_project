...
class SellerChatWidget extends StatefulWidget {
  const SellerChatWidget({Key? key}) : super(key: key);

  @override
  // ignore: library_private_types_in_public_api
  _SellerChatWidgetState createState() => _SellerChatWidgetState();
}
...
class _SellerChatWidgetState extends State<SellerChatWidget> {
  ...
  Future<void> _loadChatUsers() async {
    ...
    if (user != null) {
      ...
      final chatUsers = await _firestore.collection('users').get();

      final filteredBuyers = chatUsers.docs
          .where((doc) =>
              doc['isSeller'] == false &&
              doc.id !=
                  sellerId) // Filter buyers who are not sellers and exclude the current user
          .map((doc) {
        final userId = doc.id;
        final username = doc['fullName'];
        final profileImageUrl = doc['profileImage'];
      ...
      setState(() {
        _chatUsers.addAll(filteredBuyers);
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _chatUsers.isEmpty
          ? const Center(
              child: Text(
                'You do not have any message at the moment',
  ...
  Stream<int> _getUnreadMessagesCountStream(ChatUser chatUser) {
    final user = _auth.currentUser;
    if (user != null) {
      final chatRoomId = _getChatRoomId(chatUser.userId, user.uid);
